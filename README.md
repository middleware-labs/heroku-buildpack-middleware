...
![Middleware](https://img.shields.io/badge/Middleware-0078D4?style=for-the-badge&logo=middleware&logoColor=white)
...
Below is the list of all environment variables that you can set in your Heroku application to change the behavior of the Middleware buildpack.
...
| `MW_CONFIG_FILE`                   | No       | Location of the configuration file for the Middleware agent. Defaults to `/app/mw-agent/etc/mw-agent/agent-config.yaml` on Linux/Darwin or `agent-config.yaml` on Windows. |
| `MW_OTEL_CONFIG_FILE`              | No       | Location of the OTEL pipelines configuration file for the agent. Defaults to `app/mw-agent/etc/mw-agent/otel-config.yaml`.|
...
When you set `MW_DYNO_HOSTNAME` is set to `true` in the buildpack configuration, `mw-agent` reports the hostname in appname.dyno format (e.g. `myapp.web.1`, `myapp.run.2`). This allows for continuity in Dyno metric monitring giving accurate picture of your Heroku application performance.
